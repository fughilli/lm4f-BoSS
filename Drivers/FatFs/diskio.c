...
static void (*DLY10U)(void);  //delay 100 microseconds
static BYTE (*rcvr_spi)(void);
static BYTE (*xmit_spi)(BYTE);
static void (*set_init_speed)(void);
...
static volatile
unsigned long Timer1, Timer2;    /* 100Hz decrement timer */
...
void attach_dly10u (void (*dly10u)(void))
{
	DLY10U  = dly10u;
}

/*-----------------------------------------------------------------------*/
/* Attach the SPI functions from fatfs.cpp                   */
/*-----------------------------------------------------------------------*/
void attach_SPIdriver(BYTE (*spi_receive)(void), BYTE (*spi_send)(BYTE) , void (*spi_init_speed)(void), void (*spi_high_speed)(void))
{
	rcvr_spi = spi_receive;
	xmit_spi = spi_send;
	set_init_speed = spi_init_speed;
	set_max_speed = spi_high_speed;
}
...
BYTE wait_ready (void)
{
	...
	for(Timer2 = 50000;Timer2>0 && (res != 0xFF);Timer2--){
		res = rcvr_spi();
		DLY10U();}//delayMicroseconds(10);}
...
void send_initial_clock_train(void)
{
	...
	for (i = 10; i>0; i--) rcvr_spi();	/* 80 dummy clocks with CS=H */
...
)
{
	...
	for(Timer1 = 10000; Timer1 > 0 && (token == 0xFF); Timer1--){
		token = rcvr_spi();
		DLY10U();}//delayMicroseconds(10);}
